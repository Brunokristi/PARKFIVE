...
const colors = computed(() => {
    const lightcolor = getProjectColor('lightcolor', '#95bea7')
    const darkcolor = getProjectColor('darkcolor', '#2f543f')
    ...
    return {
        background,
        foreground,
        road: isLight.value ? '#d7e6dc' : '#6f8c79',
        roadStrong: isLight.value ? '#c5d9cc' : '#5f7c69',
        water: isLight.value ? '#9fc8b5' : '#284936',
...
const mapStyles = computed<any[]>(() => {
    ...
    return [
        ...
        { elementType: 'labels.text.fill', stylers: [{ color: c.foreground }] },
...
function createHomeMarkerIcon() {
    ...
    const svg = `
        <svg width="28" height="28" viewBox="0 0 87 155" xmlns="http://www.w3.org/2000/svg">
            <path
...
watch(
    ...
    () => {
        ...
        map.setOptions({ styles: mapStyles.value })
...
<template>
  <div class="relative h-full w-full overflow-hidden">
    ...
    <div
      class="pointer-events-none absolute inset-0 opacity-10 mix-blend-multiply"
      :style="overlayStyle"
    ></div>
